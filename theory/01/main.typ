...
two central concepts in data networking.
...
+ Suppose $d_"prop"$ is greater than $d_"trans"$.
  At time $t = d_"trans"$, where is the first bit of the packet?

+ Suppose $d_"prop"$ is less than $d_"trans"$.
  At time $t = d_"trans"$, where is the first bit of the packet?

+ Suppose $s = 2.5 dot 10^8$, $L = 120$ bits, and $R = 56$ kbps.
...
in which an example is provided with a 1 Mbps link.
Users are generating data at a rate of 100 kbps when busy,
but are busy generating data only with probability $p = 0.1$.
...
+ Suppose $N$ packets arrive simultaneously to a link
  at which no packets are currently being transmitted or queued.
...
#solution[
  ...
  + It takes exactly $N L / R$ seconds to transmit a batch of $N$ packets.
    ...
    Therefore, the average queuing delay across all batches is the same as
    the average queuing delay for a single batch, which is $((N - 1) L) / (2 R)$ seconds.
]
...
Assume all other links have abundant capacity and that there is no other traffic in the network
besides the traffic generated by the $M$ client-server pairs.
...
#solution[
  ...
  + 800000 bits.
    This is because the file size is smaller than the bandwidth-delay product.
...
the source host segments long, application-layer messages (for example, an image or a music file)
into smaller packets and sends the packets into the network.
...
+ Consider sending the message from source to destination without message segmentation.
  ...
  Keeping in mind that each switch uses store-and-forward packet switching,
...
+ Now suppose that the message is segmented into 800 packets, with each packet being 10000 bits long.
  ...
  When the first packet is being sent from the first switch to the second switch,
  the second packet is being sent from the source host to the first switch.
...
#solution[
  ...
  + $d_"file" = 802 d_21 = 4.01$ seconds, way smaller than the first answer.
    This shows that message segmentation is more efficient.

  + If an error occurs during transmission, only the affected segment needs to be resent,
    rather than the entire large message.
